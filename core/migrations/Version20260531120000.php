    ...
    public function up(Schema $schema): void
    {
        ...
        if (!$schema->hasTable('game_plugins')) {
            if ($platform instanceof SQLitePlatform) {
                ...
                $this->addSql('CREATE UNIQUE INDEX uq_game_plugins_template_name_version ON game_plugins (template_id, name, version)');
            } else {
                $this->addSql('CREATE TABLE game_plugins (id INT AUTO_INCREMENT NOT NULL, template_id INT NOT NULL, name VARCHAR(160) NOT NULL, version VARCHAR(80) NOT NULL, checksum VARCHAR(128) NOT NULL, download_url VARCHAR(255) NOT NULL, description LONGTEXT DEFAULT NULL, extract_subdir VARCHAR(128) DEFAULT NULL, install_mode VARCHAR(32) NOT NULL, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, INDEX idx_game_plugins_template (template_id), UNIQUE INDEX uq_game_plugins_template_name_version (template_id, name, version), PRIMARY KEY (id)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB');
        ...
        $this->addSql(sprintf(
            'INSERT INTO game_plugins (template_id, name, version, checksum, download_url, description, extract_subdir, install_mode, created_at, updated_at) SELECT legacy.template_id, MIN(legacy.name), legacy.version, MIN(legacy.checksum), MIN(legacy.download_url), MIN(%s), MIN(%s), MIN(%s), MIN(legacy.created_at), MAX(legacy.updated_at) FROM game_template_plugins legacy WHERE NOT EXISTS (SELECT 1 FROM game_plugins current_plugin WHERE current_plugin.template_id = legacy.template_id AND LOWER(current_plugin.name) = LOWER(legacy.name) AND current_plugin.version = legacy.version) GROUP BY legacy.template_id, LOWER(legacy.name), legacy.version',
            $description,
            $extractSubdir,
            $installMode,
        ));
