    ...
    public function overview(Request $request): Response
    {
        ...
        return new Response($this->twig->render('admin/backups/index.html.twig', [
            ...
            'backups' => array_map(fn (Backup $backup): array => [
                ...
                'target_type' => $backup->getDefinition()->getBackupTarget()?->getType()->value ?? 'local',
                'target_path' => (string) (($backup->getDefinition()->getBackupTarget()?->getConfig() ?? [])['remote_path'] ?? ($backup->getDefinition()->getBackupTarget()?->getConfig() ?? [])['root_path'] ?? ($backup->getDefinition()->getBackupTarget()?->getConfig() ?? [])['base_path'] ?? ''),
