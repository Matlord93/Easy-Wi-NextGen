...
final class AdminJobController
{
    ...
    public function create(Request $request): Response
    {
        ...
        $job = new \App\Module\Core\Domain\Entity\Job($formData['type'], $formData['payload']);
    ...
    public function retry(Request $request, string $id): Response
    {
        ...
        $retryJob = new \App\Module\Core\Domain\Entity\Job($job->getType(), $job->getPayload());
    ...
    private function normalizeJob(\App\Module\Core\Domain\Entity\Job $job): array
    {
        return [
            'id' => $job->getId(),
            'type' => $job->getType(),
    ...
    private function normalizeResult(\App\Module\Core\Domain\Entity\JobResult $result): array
    {
        return [
    ...
    private function extractLogLines(\App\Module\Core\Domain\Entity\Job $job): array
    {
        $result = $job->getResult();
        if ($result === null) {
            return [];
