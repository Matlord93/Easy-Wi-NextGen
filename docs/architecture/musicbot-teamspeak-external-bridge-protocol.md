...
- `backend_type` selects the bridge-side adapter (`placeholder`, `native_sdk`, `client_library`, or `disabled`).
- `backend_path` is validated by guarded bridge-side SDK/library stubs before use; no shell execution is allowed.
