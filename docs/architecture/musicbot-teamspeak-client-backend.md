...
The Easy-Wi bridge itself selects one of these adapter layers from its connect request:

- `placeholder`: safe default; never reports ready.
- `native_sdk`: guarded stub unless official SDK/library bindings are present and allowed.
- `client_library`: guarded stub unless an allowed TeamSpeak client library binding is present.
- `disabled`: explicit no-op backend.

Configuration fields include `backend_type`, `host`, `port`, `nickname`, `identity_path`, `channel_id`, optional `backend_path`, and secret-managed `server_password` / `channel_password`. Runtime `external_client_bridge` also supports `bridge_backend_type` / `client_backend_type` and `client_library_path` / `native_sdk_path` to pass the desired bridge-side client layer to `easywi-teamspeak-bridge`.
...
This is the currently implemented TeamSpeak client-layer integration path. The runtime starts the configured bridge executable directly (no shell) and communicates over newline-delimited JSON on stdin/stdout. The bridge protocol supports `connect`, `disconnect`, `join_channel`, `send_opus_frame`, `status`, `reconnect`, and `shutdown`. Opus frames are base64 encoded in `send_opus_frame` requests. The bridge can select `placeholder`, `native_sdk`, `client_library`, or `disabled`; `native_sdk` and `client_library` currently validate their configured paths and fail clearly instead of claiming a fake connection when no allowed library binding is compiled in.
