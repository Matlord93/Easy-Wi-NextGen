...
4. Lokal Safety-Checks ausführen:
   ```bash
   cd core
   php bin/console doctrine:schema:validate --skip-sync --env=test --no-interaction
   ```
   Dieser Mapping-only-Check darf weiterhin ohne echte Datenbank-Synchronisierung laufen, weil `--skip-sync` nur die Doctrine-Mappings validiert.

5. Für einen echten Migrations- und Schema-Sync-Check muss MariaDB/MySQL verwendet werden, z. B.:
   ```bash
   cd core
   APP_ENV=test DATABASE_URL="mysql://easywi:easywi@127.0.0.1:3306/easywi_test?serverVersion=mariadb-10.11.0&charset=utf8mb4" php bin/console doctrine:migrations:migrate --env=test --no-interaction --allow-no-migration
   APP_ENV=test DATABASE_URL="mysql://easywi:easywi@127.0.0.1:3306/easywi_test?serverVersion=mariadb-10.11.0&charset=utf8mb4" php bin/console doctrine:schema:validate --env=test --no-interaction
   APP_ENV=test DATABASE_URL="mysql://easywi:easywi@127.0.0.1:3306/easywi_test?serverVersion=mariadb-10.11.0&charset=utf8mb4" php bin/console doctrine:schema:update --dump-sql --env=test --no-interaction
   ```
   `doctrine:schema:update --dump-sql` darf nach den Migrationen keinen ausstehenden SQL-Diff ausgeben.

SQLite ist für die konsolidierte Initialmigration ungeeignet: `core/migrations/Version20250101000000.php` enthält MySQL/MariaDB-spezifisches SQL wie `AUTO_INCREMENT`, Inline-`INDEX`-Definitionen, `JSON`, `DEFAULT CHARACTER SET`, `COLLATE` und `ENGINE = InnoDB`. Deshalb müssen CI und lokale Sync-Checks dieselbe Datenbankplattform verwenden wie die Migrationen.
...
- Jede Migrations-PR muss CI-Safety-Checks erfolgreich durchlaufen:
  - Mapping-only: `doctrine:schema:validate --skip-sync`
  - Migrationen gegen MariaDB/MySQL: `doctrine:migrations:migrate --allow-no-migration`
  - Schema-Sync gegen MariaDB/MySQL: `doctrine:schema:validate`
  - Optionaler Diff-Check gegen MariaDB/MySQL: `doctrine:schema:update --dump-sql` ohne ausstehende SQL-Ausgabe
