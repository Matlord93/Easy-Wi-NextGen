...
3. **Go Tests grün**
   ...
   - Ein 403-Fehler von `proxy.golang.org` ist ein Netzwerk-/Cache-Problem, kein Code-Fehler. Sobald die Module über Cache, erlaubten Direktzugriff oder einen internen Mirror geladen wurden, laufen die Unit Tests ohne echte Discord-/TeamSpeak-Verbindungen.
   - CI-Workflows setzen `GOPROXY=https://proxy.golang.org,direct`, zeigen `go env GOPROXY`, laden Module mit `go mod download`, prüfen sie mit `go mod verify` und cachen `~/go/pkg/mod` nach `agent/go.sum`-Hash; nach dem ersten Lauf ist normalerweise kein erneuter Download nötig. Falls Runner sowohl `proxy.golang.org` als auch direkte GitHub-Downloads blockieren, muss ein interner Go-Module-Mirror vorangestellt werden, z. B. `GOPROXY=https://<internal-mirror>,https://proxy.golang.org,direct`.
4. **PHPUnit grün**
